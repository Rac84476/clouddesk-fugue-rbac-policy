pipeline {
  ...
  stages {
    ...
    stage("Apply Policy") {
      ...
      steps {
        script {
          def ret = sh(script: "curl -s -X POST 'https://$EWC_DNSNAME/oidc/token' -H 'content-type: application/x-www-form-urlencoded' --data 'username=jonathan@fugue.co&password=asdfasdf&client_id=fugue_enterprise_web_console&grant_type=password'", returnStdout: true)
          echo ret
        }
      }
    }
